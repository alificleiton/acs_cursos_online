...
<?php 
  ...
  while($res = mysqli_fetch_array($result)){
        ...
        $cidade = $res["cidade"];
        $estado = $res["estado"];
        $especificacoes = $res["especificacoes"];
...
<div class="container ml-4">
  ...
  <form class="mr-4" method="post">
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        <label for="inputEmail4" class="text-light">Especificações/Formação</label>
        <input type="text" class="form-control text-dark" name="esp" value="<? echo $especificacoes; ?>">
      </div>
      <div class="form-group col-md-6">
        <label for="inputBiografia4" class="text-light">Biografia</label>
        <input type="text" class="form-control text-dark" name="des"  value="<? echo $descricao; ?>">
      </div>
    </div>
...
<?php
  ...
  if(isset($_POST['atualizar'])){
    ...
    $caminho = '../imagens/perfil/' .$_FILES['foto']['name'];
    $imagem = $_FILES['foto']['name']; 
    $imagem_temp = $_FILES['foto']['tmp_name']; 
    move_uploaded_file($imagem_temp, $caminho);


    echo "<script language='javascript'>window.location= 'painel_professor.php?acao=perfil&cpf=$cpf&img=$imagem'</script>";
...
<?php
  ...
  if(isset($_POST['salvar'])){
    ...
    $especificacoes_post = $_POST['esp'];
    $descricao_post = $_POST['des'];

  

  if($res["cpf"] != $cpf_post){
      //VERIFICAR SE O CPF JÁ ESTÁ CADASTRADO
    $query_verificar_cpf = "SELECT * from professores where cpf = '$cpf_post' ";
    ...
    $query_verificar_usu = "SELECT * from professores where email = '$email_post' ";
    ...
    $query_post = "UPDATE professores SET nome = '$nome_post', telefone = '$telefone_post', cpf = '$cpf_post', email = '$email_post', senha = '$senha_post', endereco = '$endereco_post', cidade = '$cidade_post', estado = '$estado_post', especificacoes = '$especificacoes_post', descricao = '$descricao_post' , imagem = '$foto' where cpf = '$res[cpf]' ";
    ...
    echo "<script language='javascript'>window.location='painel_professor.php?acao=perfil&cpf=$cpf_post'; </script>";
